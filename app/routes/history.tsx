...
export function meta({}: Route.MetaArgs) {
  return [
    { title: "History | Solenis Calculations" },
    {
      name: "description",
      content:
        "Previous calculation submissions with approval status and reviewer details.",
    },
  ];
}
      ...
      <main className="mx-auto w-full max-w-6xl px-4 py-10 sm:px-6 sm:py-14">
        <div className="mb-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
          ...
          <p className="mt-2 max-w-2xl text-muted-foreground">
            Previous submissions with approval status, reviewer, and Melbourne
            timestamps.
          </p>
        </div>
                      ...
                      <dl className="mt-3 grid gap-2 text-sm sm:grid-cols-2">
                        ...
                        {run.status === "REJECTED" && run.reviewNote ? (
                          <div className="sm:col-span-2">
                            ...
                            <dd className="mt-1 font-medium">{run.reviewNote}</dd>
                          </div>
                        ) : null}
                      </dl>
                    </div>
